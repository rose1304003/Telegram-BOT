...
def chunk_messages(msgs: List[Dict], max_chars: int = 8000) -> List[str]:
    """Chunk list of messages into concatenated text blocks not exceeding max_chars."""
    blocks = []
    current = ""
    for m in msgs:
...
async def summarize_window(client, model: str, msgs: List[Dict], period_label: str) -> str:
    ...
    # Use async client if available; fallback to sync via to_thread
    try:
